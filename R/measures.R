...
setMethod("crossSupportRatio",  signature(x = "itemsets"),
function(x,  transactions = NULL, itemSupport = NULL) {
  ...
  # calculate cross support ratio
  crossSupportRatio <-  
    sapply(itemset_list, function(i) min(itemSupport[i])) /
    sapply(itemset_list, function(i) max(itemSupport[i]))

  # check for insufficient item support (NAs) 
  if(any(is.na(crossSupportRatio))) warning("some values are missing because the itemset does not contain all needed item supports. provide itemSupport or transactions.")
