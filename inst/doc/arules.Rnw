...
\author{Michael Hahsler\\Southern Methodist University \And
    Bettina Gr{\"u}n\\Johannes Keppler University Linz \AND 
...
$\le$~USD~50,000 and $>$~USD~50,000, respectively. 
The raw data is
...
Note that more support to transform continuous variables into a categorical 
variable is provided by \func{categorize}.  
